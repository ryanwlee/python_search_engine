<html>
<head>
<title>Dig Anything! DigAny!</title>
</head>

<body>
<a href="/">
<img src="https://scontent-a-lga.xx.fbcdn.net/hphotos-xpa1/v/t1.0-9/10393738_10154750525080408_7417565229542874316_n.jpg?oh=bc0a823cec3a5d642e65a6dac8ccf217&oe=5514E39A"></a>
<form action="/anonymous" method="post"><input type="text" name="keywords" size='80'><input type="submit" name="check" value="Search"><input type="submit" name="check" value="Sign In"></form>
</body>
</html>
